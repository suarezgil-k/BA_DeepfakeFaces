"""
NOTE:
This file is a modified copy of the original source file: dcbm.py
...
class CBM ():
    ...
    def construct_paths(self):
        ...
        if dataset == "places365":
            ...
            local_name = f'images_FFpp_PipelineTest_train_{model_name}_0.torch'  #EDITED
            train_path = os.path.join(embeddings_parent_directory, local_name)  #EDITED
            local_name = f'images_FFpp_PipelineTest_test_{model_name}_0.torch'  #EDITED
    ...
    def load_concepts(self, segment_path, segmentation_technique, concept_name, selected_image_concepts = None, concept_per_class = None, crop = False):
        ...
        if dataset == "FFpp_PipelineTest":
           if not crop:
               if concept_name != None:
                  concepts = f'segcrop_FFpp_PipelineTest_{seg_tech}_{concept_name}_{model_name}.torch'  #EDITED
               else:
                  concepts = f'segcrop_FFpp_PipelineTest_{seg_tech}_{model_name}.torch'  #EDITED
           else:
             self.background = True
             if concept_name != None:
                  concepts = f'segmask_FFpp_PipelineTest_{seg_tech}_{concept_name}_{model_name}.torch'  #EDITED
             else:
